...
// Resize handler
    ...
    void Obvious2D::setBorder(unsigned int pixelsLeft, unsigned int pixelsRight, unsigned int pixelsTop,
                              unsigned int pixelsBottom) {
        ...
        if (_showHelp) {
            for (int i = 0; i < _helpText.size(); i++) {
                if (_helpText[i].text.length() > 0) {
                    ...
                    glColor4ub(_helpText[i].rgbaBG[0], _helpText[i].rgbaBG[1], _helpText[i].rgbaBG[2],
                               _helpText[i].rgbaBG[3]);
                    glWindowPos2i(posX - 1, posY);
    ...
    void Obvious2D::drawInfo() const {
        ...
        for (int i = 0; i < _infoText.size(); i++) {
            if (_infoText[i].text.length() > 0) {
                ...
                glColor4ub(_infoText[i].rgbaBG[0], _infoText[i].rgbaBG[1], _infoText[i].rgbaBG[2],
                           _infoText[i].rgbaBG[3]);
    ...
    void Obvious2D::drawPopup() const {
        ...
        if (_popupText[0].text.length() > 0) {
            ...
            glColor4ub(_popupText[0].rgbaBG[0], _popupText[0].rgbaBG[1], _popupText[0].rgbaBG[2],
                       _popupText[0].rgbaBG[3]);
    ...
    void Obvious2D::draw(unsigned char *image, unsigned int width, unsigned int height, unsigned int channels) {
        ...
        glVertex3f(-1.0 + borderLeft, 1.0 - borderTop, 0.0);
        glTexCoord2f(0.0, 1.0);
        glVertex3f(-1.0 + borderLeft, -1.0 + borderBottom, 0.0);
        glTexCoord2f(1.0, 1.0);
        glVertex3f(1.0 - borderRight, -1.0 + borderBottom, 0.0);
        glTexCoord2f(1.0, 0.0);
        glVertex3f(1.0 - borderRight, 1.0 - borderTop, 0.0);
        glEnd();
    ...
    void Obvious2D::registerKeyboardCallback(const char key, fptrKeyboardCallback callback, std::string helpText,
    ...
    void Obvious2D::registerKeyboardClient(const char key, Obvious2DClient *client, std::string helpText,
                                           unsigned char rgba[4], unsigned char rgbaBG[4]) {
        _mClient[key] = client;
    ...
    void Obvious2D::addCrosshair(unsigned int x, unsigned int y, const char *text, unsigned char rgba[4],
                                 unsigned char rgbaBG[4], float radius, float offset) {
        CrossHair c;
        c.text = string(text);
        c.radius = radius;
        c.col = x;
    ...
    void
    Obvious2D::addVolatileInfoText(const char *text, unsigned char rgba[4], unsigned char rgbaBG[4], double progress,
                                   bool popup) {
        TextStruct ts;
        ts.text = string(text);
